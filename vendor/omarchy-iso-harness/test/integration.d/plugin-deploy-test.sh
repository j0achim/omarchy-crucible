...
if [[ $HANDOFF == "true" && $FAILURES == 0 ]]; then
  log "Handoff: connect with any VNC viewer, e.g. \`vncviewer 127.0.0.1:0\` or \`remote-viewer vnc://127.0.0.1:5900\` (no password)"
  read -r -p "Press Enter here when you're done inspecting the VM to tear it down... "
fi
