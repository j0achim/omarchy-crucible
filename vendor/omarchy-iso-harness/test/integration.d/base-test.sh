...
start_vm_from_base() {
  ...
  # Crucible addition: forward extra args (e.g. -vnc :0 for --handoff) to
  # the underlying start_vm — upstream never needed this, since none of its
  # scenarios export a display.
...
# Crucible addition: upstream base-test.sh has no file-copy helper.
scp_to_guest() {
  local src="$1" dest="$2"
  scp -i "$SSH_KEY" -P "$SSH_PORT" -r \
    ...
    -o ConnectTimeout=5 \
    -o LogLevel=ERROR \
    "$src" "$GUEST_USER@127.0.0.1:$dest"
}

# Crucible addition: omarchy-shell needs $OMARCHY_PATH, which a bare
# non-login ssh_guest exec doesn't have (profile scripts never run) — see
# the omarchy-cidata-autologin note. Route every omarchy-shell/omarchy CLI
# call through a login shell instead of calling ssh_guest directly.
omarchy_cli() {
...
install_phase() {
  ...
  # Crucible addition: a normal (unencrypted) cidata install deliberately
  # does not autologin — omarchy-iso's own orchestrator only autologins
  # encrypted installs, reasoning that the LUKS prompt already proves
  # presence (see the omarchy-cidata-autologin vault note). That's correct
  # for a real machine and wrong for a disposable, always-fresh test VM
  # with nothing on it worth protecting — bake autologin into the base
  # image once here rather than the SDDM greeter blocking every ephemeral
  # run's IPC calls.
  log "Enabling autologin for this disposable test VM (baked into the base image, not a real install default)"
